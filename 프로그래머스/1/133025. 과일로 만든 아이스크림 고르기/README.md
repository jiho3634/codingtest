...
## 포인트
JOIN과 ON
JOIN을 단독으로 사용하면, 이는 대부분의 SQL 데이터베이스에서 INNER JOIN을 의미합니다. 

JOIN의 기본 유형들:
INNER JOIN: 두 테이블 모두에서 일치하는 데이터만 반환합니다.
LEFT (OUTER) JOIN: 왼쪽 테이블의 모든 레코드와 오른쪽 테이블의 일치하는 레코드를 반환합니다. 오른쪽 테이블에 일치하는 데이터가 없으면 NULL로 채워집니다.
RIGHT (OUTER) JOIN: 오른쪽 테이블의 모든 레코드와 왼쪽 테이블의 일치하는 레코드를 반환합니다. 왼쪽 테이블에 일치하는 데이터가 없으면 NULL로 채워집니다.
FULL (OUTER) JOIN: 두 테이블의 모든 레코드를 반환하며, 한쪽 테이블에만 존재하는 데이터는 다른 쪽은 NULL로 채워집니다.
CROSS JOIN: 두 테이블의 모든 가능한 조합을 반환합니다.

ON절은 JOIN 조건을 명시하여 두 테이블의 관련된 데이터만을 선택적으로 합치게 합니다.

작동 원리:
조건 검사: 쿼리는 FIRST_HALF 테이블의 각 행에 대해 ICECREAM_INFO 테이블을 검색하여, FLAVOR 값이 일치하는 행을 찾습니다.
결과 생성: 일치하는 FLAVOR 값을 가진 행들은 결합되어 새로운 결과 행을 형성합니다. 이렇게 조합된 결과 행은 두 테이블의 데이터를 포함합니다.
